...
export default function AdminVisitsStats() {
  ...
  useEffect(() => {
    if (!open) return;
    setLoading(true);
    fetch(`/api/admin/visits/stats?days=${range}`)
      .then((r) => r.json())
  ...
  const maxDay = Math.max(...stats.map((s) => s.unique), 1);
  const totalPeriod = stats.reduce((a, s) => a + s.unique, 0);

  // Today & this month
  const today = new Date().toISOString().slice(0, 10);
  const thisMonth = today.slice(0, 7);
  const todayCount = stats.find((s) => s.day === today)?.unique ?? 0;
  const monthCount = byMonth.get(thisMonth) ?? 0;
      ...
      {open && (
        <div className="mt-4 rounded-2xl border border-border bg-surface p-5">
          ...
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-muted">
                Visitantes nuevos por día (por primera visita registrada)
              </p>
            </div>
          ...
          {loading ? (
            ...
            <p className="py-8 text-center text-sm text-muted">Sin datos para este período.</p>
          ) : (
            <>
              {/* KPIs */}
              <div className="mb-4 grid grid-cols-3 gap-3">
                {[
                  ...
                  { label: `Últimos ${range === 365 ? "365" : range} días`, value: totalPeriod },
                ].map((k) => (
              ...
              <h3 className="mb-2 text-xs font-semibold text-muted uppercase tracking-wide">Por día</h3>
              ...
              <div className="mt-1 flex justify-between text-[10px] text-muted">
                <span>{fmtDay(stats[0].day)}</span>
                <span>{fmtDay(stats[stats.length - 1].day)}</span>
              </div>

              {/* Por mes */}
              {months.length > 0 && (
                <>
                  <h3 className="mb-2 mt-5 text-xs font-semibold text-muted uppercase tracking-wide">Por mes</h3>
